...
`N`은 `kMaxDailyTarget = 40`으로 자른다 — 따라잡기 산식은 시험이 가까울수록 폭발해서(D-1에 1,000개가 남으면 1,000) 하루에 끝낼 수 없는 세트를 만들기 때문이고, 넘치는 분량은 다음 날로 넘어간다. 또한 구현은 남은 단어가 없을 때 `max(1, …)` 대신 `0`을 돌려준다(할 게 없으면 세트도 비어야 한다).
